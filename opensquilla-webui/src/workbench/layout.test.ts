...
describe('workbench layout', () => {
  ...
  it('does not overwrite the desktop preference when resolving narrow modes', () => {
    const preference = { version: 1 as const, width: 600 }
  ...
  it('safely parses versioned width storage', () => {
    expect(parseWorkbenchWidthPreference(null).width).toBe(WORKBENCH_DEFAULT_WIDTH)
    expect(parseWorkbenchWidthPreference('invalid').width).toBe(WORKBENCH_DEFAULT_WIDTH)
