...
def repair_only_resume_classification_audit(
    *,
    ...
    allowed_actions = {"complete", "judge_only", "metadata_only", "policy_violation"}
...
def write_manifest(
    ...
    *,
    ...
    repair_compatibility_audit = getattr(args, "_repair_compatibility_audit", None)
    if isinstance(repair_compatibility_audit, dict):
        payload["repair_compatibility_audit"] = repair_compatibility_audit
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Allow only source_identity drift from --expected-compatibility-manifest "
            "for a bounded resume repair wave; requires clean source, resume JSONL, "
            "and an explicit group/task key file, and refuses every generation path."
        ),
    )
    parser.add_argument(
