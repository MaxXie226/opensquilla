...
readonly REPORT_ROOT="/home/codex/code/opensquilla-agentic-routing/reports/draco"
readonly REFERENCE_REPO="/home/codex/code/opensquilla"
...
capture_stable_after() {
  ...
  local settlement_started_epoch elapsed_seconds
  local attempt poll_path usage byok
  settlement_started_epoch="$(date +%s)"
  for attempt in $(seq 1 120); do
    poll_path="$ACCOUNT_POLL_DIR/poll-$(printf '%03d' "$attempt").json"
    capture_account_snapshot "$poll_path"
    ACCOUNT_POLL_FILES+=("$poll_path")
    usage="$(jq -r '.usage' "$poll_path")"
    byok="$(jq -r '.byok_usage' "$poll_path")"
    if [[ -n "$previous_usage" && "$usage" == "$previous_usage" && "$byok" == "$previous_byok" ]]; then
      stable_streak=$((stable_streak + 1))
    else
      stable_streak=1
    fi
    elapsed_seconds=$(( $(date +%s) - settlement_started_epoch ))
    if [[
    ...
    previous_byok="$byok"
    sleep "$ACCOUNT_SETTLEMENT_POLL_SECONDS"
  done
  if [[
  ...
  install -m 600 "${ACCOUNT_POLL_FILES[-1]}" "$ACCOUNT_AFTER"
  write_account_reconciliation
  ACCOUNT_SETTLED=1
}
